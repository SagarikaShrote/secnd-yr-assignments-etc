...
short int nextfit(int size)
{
	...
	if(freel!=NULL)
	{
		...
		if(allocl!=NULL)
		{
			...
			/*if(t!=NULL && t->start==x+1){			//for the assumption that the new block searched will be the next block
				t=t->next;
			}*/
			lnext=t;
		}
		...
		if(lnext==NULL)												//to give it a circular effect
		{															//if next fit starts searching from betn and ends so it should be started again from start 
			prev=NULL;
			lnext=freel;
		}
		while ((lnext!=NULL) && (lnext!=t) && (((lnext->end)-(lnext->start)+1)<size))
		{																			//automatically takes care of heap full condn
			prev=lnext;
			lnext=lnext->next;
		}
		if (lnext!=NULL && lnext==t && (((lnext->end)-(lnext->start)+1)<size))					//condn where the space is less
		{
			lnext=NULL;
			prev=NULL;
		}
