...
//Search Menu		// ***** Change this Function ***** //
...
{       //s is the no. of items to be added
	...
	{
		data = countMenuNodes(ptr);		// **********Need function to count number of keys********** //
	}
        char name[20];
        while(i<s){
                i++;
		data++;
                ...
                printf("input price\n");
                scanf("%d",&price);
                menu* temp = MakeNode_Menu(name,price,data);
		ptr = Insert(ptr,temp);
		printTree(ptr);
        }
        return ptr;
...
user * user_search(user* root,int p){
	user * ret=NULL;
	if(ulist!=NULL){
		if(root->pno==p){
			ret=root;
			...
			ret=user_search(root->next,p);
			if(ret==NULL){
				ret=user_search(root->branch0,p);
			}
			if(ret==NULL && root->next==NULL){
				ret=user_search(root->branch1,p);
			}
		}
	}
	return ret;
...
{
	int num = countMenuNodes(Menu)+1;
	menu* mptr = MakeNode_Menu(Menu->item,Menu->price,num);		// **********Need function to count number of keys********** //
	return mptr;
}
...
void order(rest* lptr)
{
	...
	scanf("%d",&num);
	user* nptr = user_search(ulist,num);		// ***** User Search number se banana padega. ***** //
	if(nptr== NULL)
	{
